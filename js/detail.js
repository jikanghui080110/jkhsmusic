...
// 替换原有的 renderDetail 函数
function renderDetail(song) {
  // 1. 填充文本信息
  document.getElementById("display-title").textContent = song.title || "未命名歌曲";
  document.getElementById("display-artist").textContent = song.artist || "未知歌手";
  document.getElementById("display-review").innerHTML = escapeHtml(song.review || "暂无评价").replace(/\n/g, '<br>'); // 支持简单的换行

  // 2. 渲染雷达图和详细分数到右侧容器
  const chartContainer = document.getElementById("chart-container");
  chartContainer.innerHTML = `
    <div class="radar-wrap" style="display: flex; flex-direction: column; align-items: center; width: 100%;">
      ${buildRadarSvg(song)}
      <div class="score-list" style="margin-top: 20px; width: 100%;">
        ${buildScoreRows(song)}
      </div>
    </div>
  `;
